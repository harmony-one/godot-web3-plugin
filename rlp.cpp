...
PackedByteArray RLP::encode_length(int length, int offset) {
  PackedByteArray out;
  if (length < 56) {
    ...
    PackedByteArray length_bin = StringUtils::hex_to_bytes(length_hex);
    // convert prefix to bytes
    int prefix = offset + 55 + length_bin.size();
    String prefix_hex = String::num_int64(prefix, 16);
    PackedByteArray prefix_bin = StringUtils::hex_to_bytes(prefix_hex);
...
PackedByteArray RLP::encode(const String &p_hex) {
  PackedByteArray out;
  PackedByteArray bin = StringUtils::hex_to_bytes(p_hex);
